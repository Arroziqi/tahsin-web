...
import AcademicCalendarTable from '@/components/table/AcademicCalendarTable';
import useAcademicCalendars from '@/hooks/fetchData/useAcademicCalendars';

function AcademicCalendarPage() {
  const { data, loading, error, refresh, setError } = useAcademicCalendars();

  return (
    <div className="relative z-1 w-full flex justify-center items-center flex-col bg-white">
      <Topbar title="Kelola Kalender Akademik" />
      <div className="w-full h-screen overflow-y-auto">
        <div className="mx-auto pt-[103px] flex flex-col gap-5 max-w-[936px] w-full h-fit pb-9">
          <TitlePage title="Daftar Kalender Akademik" />

          {/* Statistics Cards */}
          <CardView className="w-fit flex flex-row gap-16">
            <StatsWithIcon
              data={data.length.toString()}
              label="Total Academic Calendar"
          ...
          <AcademicCalendarTable
            ...
            loading={loading}
            error={error}
            refreshAcademicCalendars={refresh}
          />
        </div>
      </div>
